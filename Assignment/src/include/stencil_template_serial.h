...
int initialize(int, char **, int *, int *, int *, int *, int **, double *,
               double **, int *, int *);
...
inline int inject_energy(const int periodic, const int Nsources,
                         ...
                         const int mysize[2], double *plane) {
...
inline int update_plane(const int periodic, const int size[2],
                        const double *old_point, double *new_point)
/*
...
#define IDX(i, j) ((j) * fxsize + (i))
  ...
  for (int j = 1; j <= ysize; j++)
    for (int i = 1; i <= xsize; i++) {
      ...
      double alpha = 0.6;
      double result = old_point[IDX(i, j)] * alpha;
      double sum_i = (old_point[IDX(i - 1, j)] + old_point[IDX(i + 1, j)]) /
                     4.0 * (1 - alpha);
      double sum_j = (old_point[IDX(i, j - 1)] + old_point[IDX(i, j + 1)]) /
                     4.0 * (1 - alpha);
      result += (sum_i + sum_j);
  ...
  #define alpha_guess 0.5 // mimic the heat diffusivity
        ...
        double sum = old_point[IDX(i, j)];

        int done = 0;
        do {
          double sum_i = alpha * (old_point[IDX(i - 1, j)] +
                                  old_point[IDX(i + 1, j)] - 2 * sum);
          double sum_j = alpha * (old_point[IDX(i, j - 1)] +
                                  old_point[IDX(i, j + 1)] - 2 * sum);
...
inline int get_total_energy(const int size[2], const double *plane,
                            double *energy)
/*
