...
#pragma omp for schedule(guided) reduction(+ : S)
  ...
  for (int j = 0; j < 3; j++) {
    ...
    for (int i = 1; i < nthreads; i++) {
      ...
      min_timings[j] =
          (min_timings[j] < timings[j][i]) ? min_timings[j] : timings[j][i];
    }
  ...
  printf("\ntimings for the constant work case:\n"
         ...
         timings[STATIC][0], min_timings[STATIC], timings[DYNAMIC][0],
         min_timings[DYNAMIC], timings[GUIDED][0], min_timings[GUIDED]);
