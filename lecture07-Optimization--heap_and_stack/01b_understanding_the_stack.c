...
int main(int argc, char **argv) {
  ...
  register long long int myRBP __asm__(
      "rbp"); // this way you can directly access a CPU register
  register long long int myRSP __asm__(
      "rsp"); // BP is the Base of the Stack and SP is the Top of the Stack
              //
  ...
  for (int ii = 0; ii < argc; ii++) {
    printf("\targ%d's pointer resides at %p [ %zd bytes-offset from RSP]\n"
           ...
           ii, argv + ii, (void *)(argv + ii) - (void *)myRSP, ii, *(argv + ii),
  ...
  printf("\tadd_4bytes_to_stack resides at %p [ %zd bytes-offset from RSP]\n"
         ...
         &add_8bytes_to_stack, (void *)&add_8bytes_to_stack - (void *)myRSP);
...
//
{
  ...
  printf(
      ...
      &add_4bytes_to_stack, (void *)&add_4bytes_to_stack - (void *)myRSP, myRDI,
      myRSI, (void *)myRDX, this_string_goes_in_initialized_global_data);

  //  printf("%s\n", data.tag_for_this_data );

  function2(1, 2, this_string_goes_in_initialized_global_data, 1.0, 2.0,
            &this_double_goes_in_initialized_global_data, 3.0);

  return 0;
}

double function2(int iarg1, int iarg2, char *sarg3, double darg1, double darg2,
                 double *dparg1, double darg3)
//
// n.b. just get the point.. the exact placement in registers is
//      compiler dependent
//
{
  ...
  printf("------------------------------\n"
         ...
         __FUNCTION__, (void *)myRBP, (void *)myRSP,
         (void *)myRBP - (void *)myRSP, &add_4bytes_to_stack,
         (void *)&add_4bytes_to_stack - (void *)myRSP, &double_val,
         (void *)&double_val - (void *)myRSP, myRDI, myRSI, (void *)myRDX,
         this_string_goes_in_initialized_global_data, myXMM0, myXMM1,
         (void *)myRCX, dparg1, myXMM2);
