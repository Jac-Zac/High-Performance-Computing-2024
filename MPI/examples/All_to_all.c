...
/* ·························································································
 *
 *  This code provides and example on the usage of MPI_Alltoall and on how to
 * achieve the same result with 1)  a combination of Isend / Irecv 2)  a
 * combination of Isend / Recv( MPI_ANY_SOURCE )
 *
...
int main(int argc, char **argv) {
  ...
  data_t *mydata = (data_t *)malloc(
      2 * sizeof(data_t) * Ntasks);  // we allocate 2 buffers: the first half
  data_t *alldata = mydata + Ntasks; // contains the data to be sent, the second
                                     // half the data to be received
                                     //
  ...
  if (Myrank == 0)
    // NOTE: strictly speaking, we do not know whether ALL the tasks are at this
    // point; we sould need to post an MPI_Barrier, ar a collective of some kind
    // to ensure that. However, for practical purposes, letting one task to
    // issue a progression message is enough. For debugging purposes, you may
    // let all the ranks to print a message.
    //
  ...
  for (int i = 0; i < Ntasks; i++) {
    fails.sender_fails +=
        alldata[i].data[0] != i; // if the sender wasn't rank i, that's an error
    fails.receiver_fails +=
        alldata[i].data[1] != Myrank; // if I wasn't the target, that's an error
  }
  ...
  if (Myrank == 0) {
    if ((fails.sender_fails > 0) || (fails.receiver_fails > 0))
