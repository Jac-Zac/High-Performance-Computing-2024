...
#define SECOND_ROUND 1
    ...
    if (Im_even) {
      ...
      MPI_Send((void *)message, MsgSize, MPI_INT, my_buddy, FIRST_ROUND,
               MPI_COMM_WORLD);
      ...
      MPI_Recv((void *)&buddy_rank, 1, MPI_INT, my_buddy, SECOND_ROUND,
               MPI_COMM_WORLD, &status);

      if (buddy_rank == my_buddy)
        printf("[R] Rank %d says: I confirm I have received a reply from my "
               "buddy\n",
               Rank);
      else
        printf("\t[R] Rank %d says: oops, a stranger (id: %d, expected: %d) "
               "has replied to me\n",
       ...
       *
       */
      MPI_Send((void *)&Rank, 1, MPI_INT, my_buddy, SECOND_ROUND,
               MPI_COMM_WORLD);
      ...
      MPI_Recv((void *)message, MsgSize, MPI_INT, my_buddy, FIRST_ROUND,
               MPI_COMM_WORLD, &status);
      ...
      if (message[0] == my_buddy)
        ...
        printf("Rank %d says: oops, a stranger (id: %d, expected: %d) has "
