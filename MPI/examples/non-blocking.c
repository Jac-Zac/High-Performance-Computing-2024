...
int main(int argc, char **argv) {
  ...
  if (Myrank == the_sender) {
    ...
    MPI_Request *requests_size =
        (MPI_Request *)malloc(Nchunks_guess * sizeof(MPI_Request));
    MPI_Request *requests_data =
        (MPI_Request *)malloc(Nchunks_guess * sizeof(MPI_Request));

    // send the maximum size of a data chunk
    MPI_Isend(&max_chunk_size, sizeof(unsigned int), MPI_BYTE, the_receiver,
              MSG_MAXSIZE, myCOMM_WORLD, &requests_maxsize);
    ...
    while (head < data_size) {
      ...
      if (Nchunks == Nchunks_guess) {
        // if we do not have enough request handles, allocate more space for
        // them
        //
        Nchunks_guess *= 1.2;
        requests_size = (MPI_Request *)realloc(
            requests_size, sizeof(MPI_Request) * Nchunks_guess);
        requests_data = (MPI_Request *)realloc(
            requests_data, sizeof(MPI_Request) * Nchunks_guess);
      }

      // randomly determine the current chunk size
      unsigned int this_chunk_size = lrand48() % max_chunk_size;
      this_chunk_size =
          (this_chunk_size < min_chunk_size ? min_chunk_size : this_chunk_size);
      this_chunk_size = (head + this_chunk_size > data_size ? data_size - head
                                                            : this_chunk_size);

      // send the current chunk size
      MPI_Isend(&this_chunk_size, sizeof(unsigned int), MPI_BYTE, the_receiver,
                MSG_SIZE, myCOMM_WORLD, &requests_size[Nchunks]);
      ...
      MPI_Isend(&data[start], this_chunk_size, MPI_DOUBLE, 1, MSG_DATA,
                myCOMM_WORLD, &requests_data[Nchunks]);
    }
...
#define DATA_READY 2
    ...
    MPI_Recv(&data_max_size, 1, MPI_INT, the_sender, MSG_MAXSIZE, myCOMM_WORLD,
             MPI_STATUS_IGNORE);
    ...
    MPI_Recv(&chunk_size[current_chunk], 1, MPI_INT, the_sender, MSG_SIZE,
             myCOMM_WORLD, MPI_STATUS_IGNORE);

    if (chunk_size[current_chunk] > 0) {
      // if not zero, receive the data
      MPI_Recv(&data[current_chunk], chunk_size[current_chunk], MPI_DOUBLE,
               the_sender, MSG_DATA, myCOMM_WORLD, MPI_STATUS_IGNORE);
      data_status[current_chunk] = DATA_READY;

      // post the Irecv for the size of the next chunk
      MPI_Irecv(&chunk_size[!current_chunk], 1, MPI_INT, the_sender, MSG_SIZE,
                myCOMM_WORLD, &requests_size[!current_chunk]);
    ...
    while (!done) {
      ...
      while (head < chunk_size[current_chunk]) {
        ...
        stop = (stop > chunk_size[current_chunk] ? chunk_size[current_chunk]
                                                 : stop);
        for (; head < stop; head++)
          heavy_work(head);
        ...
        if (data_status[!current_chunk] != DATA_RECEIVING) {
          ...
          MPI_Test(&requests_size[!current_chunk], &other_size_received,
                   MPI_STATUS_IGNORE);
          ...
          if (other_size_received) {
            ...
            if (!done) {
              // if so, post a Irecv
              MPI_Irecv(&data[!current_chunk], chunk_size[!current_chunk],
                        MPI_DOUBLE, the_sender, MSG_DATA, myCOMM_WORLD,
                        &requests_data[!current_chunk]);
